// SnapStudio Real Application - Database Schema Fix
...
async function handleAPI(request, env, path, method) {
  ...
  if (path === '/api/appointments' && method === 'GET') {
    try {
      // Use a simpler query that works with existing schema
...
async function renderDashboard(env) {
  try {
    ...
    try {
      const clientsResult = await env.DB.prepare("SELECT COUNT(*) as count FROM clients").first();
      clientsCount = clientsResult?.count || 0;
    } catch (e) {
      console.log("Clients table might not exist yet");
    }
    
    try {
      const appointmentsResult = await env.DB.prepare("SELECT COUNT(*) as count FROM appointments").first();
      appointmentsCount = appointmentsResult?.count || 0;
    } catch (e) {
      console.log("Appointments table might not exist yet");
    }
    
    try {
      // Try to get revenue, but handle missing columns gracefully
      const revenueResult = await env.DB.prepare("SELECT SUM(COALESCE(session_fee, 0)) as total FROM appointments WHERE status = 'completed'").first();
...
<body>
    ...
    <div class="container mt-4">
        ...
        <div class="dashboard-card p-4 mt-4">
            ...
            <p>Welcome to your professional photography business management system!</p>
            <div class="row">
                <div class="col-md-6">
                    <h6>📋 Quick Setup</h6>
                    <ol>
